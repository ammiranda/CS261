...
//	assertTrue(EQ(getDynArr(dyn, 33), 29), "Test 33rd element == 29");
	...
	assertTrue(sizeDynArr(dyn) == 3, "Size decremented properly");

	printf("\n\nTesting resizing...\n");
	printf("Adding 50 elements to [6,5,9]\n");
	
	for (int i = 1; i < 51; i++) {
		pushDynArr(dyn, i);
	}

	assertTrue(containsDynArr(dyn, 50), "Test containing 50");
	assertTrue(sizeDynArr(dyn) == 53, "Size incremented properly");
